...
class Dataset(object):
    ...
    def load_data(self, w2v_file, train_file, test_file, val_file = None):
        ...
        tokenizer = lambda sent: [x for x in nltk.word_tokenize(sent) if x != " "]  # 这里是一个列表生成式，避免了 token 的结果为 " "
...
def evaluate_model(model, iterator):
    ...
    for index, batch in enumerate(iterator):
        if torch.cuda.is_available():
            ...
            x = batch.text
        y_pred = model(x)
        predicted = torch.argmax(y_pred, 1) + 1
...
def run_epoch(train_iterator, val_iterator, epoch, config, model):
    ...
    for i, batch in enumerate(train_iterator):
        ...
        # 此处得到的输入数据 x 维度是 (max_sen_len, batch_size)，而不是反过来
        y_pred = model(x)
